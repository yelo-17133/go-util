...
func TestCompare(t *testing.T) {
	expectFunc := func(a, b interface{}, opr string, expectVal, expectErr bool) {
	...
	for _, arr := range [][]interface{}{
		...
		{true, true, Operator_Gt, false, true},
		{true, true, Operator_Egt, false, true},
		{true, true, Operator_Lt, false, true},
		{true, true, Operator_Elt, false, true},
		{true, true, Operator_In, false, true},
		{true, true, Operator_NotIn, false, true},
		...
		a, b, opr, expectVal, expectErr := arr[0], arr[1], arr[2].(string), true, false
		if n := len(arr); n > 3 {
...
func TestCompareBool(t *testing.T) {
	for _, arr := range [][]interface{}{
		...
		a, b, opr, expectErr, expectVal := arr[0].(bool), arr[1].(bool), arr[2].(string), len(arr) == 3, false
		if !expectErr {
			expectVal = arr[3].(bool)
		}

		expectFunc := func(a, b bool, opr string, expectVal bool) {
...
func TestCompareInt(t *testing.T) {
	for _, arr := range [][]interface{}{
		...
		opr, expectErr, expectVal, convertErr := arr[2].(string), len(arr) == 3, false, error(nil)
		if !expectErr {
			expectVal = arr[3].(bool)
		}
		...
		expectFunc := func(a, b int64, opr string, expectVal bool) {
...
func expectFunc(t *testing.T, a, b bool, opr string, expectVal, expectErr bool) {
	val, err := Compare(a, b, opr, 0)
	if (err != nil) != expectErr {
		if expectErr {
			t.Error(fmt.Sprintf("should has error: a=%v, b=%v, opr=%v", a, b, opr))
		} else {
